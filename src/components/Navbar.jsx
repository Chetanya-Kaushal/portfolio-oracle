...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      scrolled ? 'bg-oracle-black/80 backdrop-blur-xl border-b border-hairline' : 'bg-transparent'
    }`}>
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {['Mission', 'Journey', 'Arsenal', 'Agents', 'Contact'].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} className="font-mono text-xs text-text-muted hover:text-oracle-red tracking-widest uppercase transition-colors">
              {item}
            </a>
          ))}
        </nav>
        ...
        <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden w-8 h-8 flex flex-col items-center justify-center gap-1.5 cursor-pointer">
      ...
      {menuOpen && (
        <div className="md:hidden bg-oracle-black/95 backdrop-blur-xl border-b border-hairline">
          <nav className="flex flex-col items-center gap-6 py-8">
            {['Mission', 'Journey', 'Arsenal', 'Agents', 'Contact'].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setMenuOpen(false)} className="font-mono text-sm text-text-secondary hover:text-oracle-red tracking-widest uppercase transition-colors">
                {item}
              </a>
            ))}
          </nav>
